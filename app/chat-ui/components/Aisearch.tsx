 ...
 const [uploadFileCount, setUploadFileCount] = useState(0);
  ...
  useEffect(() => {
    const fetchConversationId = async () => {
      try {
        const res = await fetchWithAuth(API_ROUTES.conversationsailegal, {
          method: "POST",
        });
  ...
  try{
     file.forEach((fileData) => {
        ...
        formData.append(`session_id`, sessionId);
         const uploadRes = await fetchWithAuth(API_ROUTES.uploadailegal, {
          method: "POST",
          body: formData,
...
const fetchAskApi = async () => {
  ...
  abortControllerRef.current = new AbortController();
try{
      const res = await fetchWithAuth(API_ROUTES.askailegal, {
        method: "POST",
